...
func TestParseUUIDv4Valid(t *testing.T) {
	...
	validShort := UUIDv4().ToShortString()
	validParsedShort, err := ParseUUID(validShort)
	assert.Nil(err)
	assert.True(validParsedShort.IsV4())
	assert.Equal(validShort, validParsedShort.ToShortString())

	validFull := UUIDv4().ToFullString()
	validParsedFull, err := ParseUUID(validFull)
	assert.Nil(err)
	...
	validParsedBracedShort, err := ParseUUID(validBracedShort)
	assert.Nil(err)
	...
	validParsedBracedFull, err := ParseUUID(validBracedFull)
	assert.Nil(err)
...
func TestParseUUIDv4Invalid(t *testing.T) {
	...
	_, err = ParseUUID("fcae3946f75d+3258678bb5e6795a6d3")
	assert.NotNil(err, "should handle invalid characters")

	_, err = ParseUUID("4f2e28b7b8f94b9eba1d90c4452")
